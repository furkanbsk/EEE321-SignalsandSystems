...
% full‐length conv outputs
y_lp_full = ConvFUNC_M(orch, h_lpf).';
y_hp_full = ConvFUNC_M(orch, h_hpf).';

% trim to 'same' length
mid = floor(length(h_lpf)/2);
y_lp = y_lp_full(mid+1 : mid+Lmax);
y_hp = y_hp_full(mid+1 : mid+Lmax);

% — play back —
soundsc(orch, fs);    disp('Playing: original orchestra');    pause(length(orch)/fs + 1);
soundsc(y_lp,  fs);    disp('Playing: after Gaussian LPF');    pause(length(y_lp)/fs + 1);
soundsc(y_hp,  fs);    disp('Playing: after Gaussian HPF');

% …after the soundsc calls…
